...
def initUpyunAuth(bucket)
    auth = $configs['auth']
    upyun = Upyun::Rest.new(bucket, auth['access_key'], auth['secret_key'])
    return upyun
end
...
##Database Related
require 'sqlite3'
DB_FILE = File.expand_path('..', __FILE__) + '/.upyun.db'
...
def uploadFile(bucket, localPath, fileName=nil, putPolicy=nil, xVar=nil)
    ...
    if $upyun.nil?
        $upyun = initUpyunAuth(bucket)
    end

    success = $upyun.put(fileName, File.new(localPath, 'rb'), {})
    puts success
